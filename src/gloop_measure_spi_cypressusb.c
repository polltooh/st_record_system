...
int32_t gloop_create_default_context_with_id(spi_context_t* context,
					     uint8_t deviceNum,
					     uint8_t interfaceNum)
{   
  return
    gloop_create_context_with_id(GLOOP_DEFAULT_VID, GLOOP_DEFAULT_PID, GLOOP_DEFAULT_FREQ_HZ, GLOOP_DEFAULT_POLARITY, GLOOP_DEFAULT_PHASE, GLOOP_DEFAULT_SELECT_ADDR, (void*) context, deviceNum, interfaceNum);
}

				     /* \brief Provide with NULL memory pointer to get allocation size needed */
int32_t gloop_create_context_with_id(uint16_t vid, uint16_t pid,
				     uint32_t spi_frequency_hz,
				     bool spi_clock_polarity,
				     bool spi_clock_phase,
				     uint8_t spi_select_address,
				     ...
				     uint8_t interfaceNum)
{
	...
	cy_vid_pid.vid = vid;
	CyLibraryInit();
	status = CyGetListofDevices(&number_of_devices_found);
	...
	for (i = 0; i < number_of_devices_found; ++i)
	{
		if (1)//(cy_device_info_array[i].deviceClass == SerialBlock_SCB0)
		{
			...
			//status = CyOpen(2, 0, &context->impl.handle);
			status = CyOpen(deviceNum, interfaceNum,
					&context->impl.handle);
			break;
		}
	}
	...
	void* memory)
{
	...
	for (i = 0; i < number_of_devices_found; ++i)
	{
		if (1)//(cy_device_info_array[i].deviceClass == SerialBlock_SCB0)
		{
			context->impl.device_index = i;
			//status = CyOpen(2, 0, &context->impl.handle);
			status = CyOpen(0, 0, &context->impl.handle);
			break;
		}
	}
...
bool gloop_read_data2(spi_context_t* context, gloop_data_spidat2_t* data_out)
{
	...
	cy_write_buf.buffer = write_struct.buf;
	cy_write_buf.length = sizeof(write_struct);
	memset(&write_struct, 0U, sizeof(gloop_data_spidat2_t));
	memset(data_out, 0U, sizeof(gloop_data_spidat2_t));
